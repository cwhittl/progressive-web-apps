if (!('serviceWorker' in navigator)) {
  define([
    './template.js', './clientStorage.js'
  ], function(template, clientStorage) {
    ...
    function fetchPromise() {
      return new Promise(function(resolve, reject) {
        fetch(apiUrlLatest + '?carId=' + clientStorage.getLastCarId() + "&description=1").then(function(response) {
    ...
    function addImages(cars) {
      for (var i = 0; i < cars.length; i++) {
        var car = cars[i];
        (function(car) {
          clientStorage.getCarImage(car.details_id).then(function(data) {
            appendImage(car.id, data);
          }).catch(function() {
            fetchImage(car);
          });
    ...
    function loadCarPage(carId) {
      clientStorage.getDetailsTemplate().then(function(data) {
        var template = data;
        clientStorage.getCar(carId).then(function(car) {
    ...
    return {loadCarPage: loadCarPage, loadMoreRequest: loadMoreRequest}
  });
}
